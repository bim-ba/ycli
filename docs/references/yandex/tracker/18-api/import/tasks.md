...
- [Формат запроса](https://yandex.ru/support/tracker/ru/api-ref/import/ru/api-ref/import/import-ticket#query)
- [Формат ответа](https://yandex.ru/support/tracker/ru/api-ref/import/ru/api-ref/import/import-ticket#answer)
...
```
https://api.tracker.yandex.net/v3/issues/_import
```
...
При необходимости вы можете расширить список полей, доступных для импорта. Для этого [создайте](https://yandex.ru/support/tracker/user/create-param.html) в Трекере дополнительные поля и укажите их в теле запроса в формате:
...
| createdBy | Логин или идентификатор автора задачи. | Строка для логинаЧисло для идентификатора |

**Дополнительные параметры**

В таблице перечислены основные параметры запроса. Полный список параметров и их описание см. на странице [Параметры запроса](https://yandex.ru/support/tracker/ru/api-ref/import/ru/api-ref/issues/request-fields).

Также вы можете использовать глобальные пользовательские поля и локальные поля очередей.

| Параметр | Описание | Тип данных |
| --- | --- | --- |
| key | Ключ задачи. Ключ должен относиться к очереди, в которую импортируется задача.Если параметр не указан, задаче будет автоматически присвоен ключ. | Строка |
| updatedAt | Дата и время последнего изменения задачи в формате YYYY-MM-DDThh:mm:ss.sss±hhmm. Вы можете указать любое значение в интервале времени от создания задачи до текущего момента.Параметр указывается только вместе с параметром updatedBy. | Строка |
| updatedBy | Логин или идентификатор пользователя, который редактировал задачу последним.Параметр указывается только вместе с параметром updatedAt. | Строка для логинаЧисло для идентификатора |
| resolvedAt | Дата и время проставления резолюции в формате YYYY-MM-DDThh:mm:ss.sss±hhmm. Вы можете указать любое значение в интервале времени от создания до последнего изменения задачи.Параметр указывается только вместе с параметрами resolution и resolvedBy. | Строка |
| resolvedBy | Логин или идентификатор пользователя, который проставил резолюцию.Параметр указывается только вместе с параметрами resolution и resolvedAt. | Строка для логинаЧисло для идентификатора |
| status | Идентификатор статуса задачи. Статус должен присутствовать в рабочем процессе очереди для выбранного типа задачи.Если статус не указан, используется начальный статус рабочего процесса. | Число |
| deadline | Дедлайн в формате YYYY-MM-DD. | Строка |
| resolution | Идентификатор резолюции задачи.Параметр указывается только вместе с параметрами resolvedBy и resolvedAt. | Число |
...
| assignee | Логин или идентификатор исполнителя. | Строка для логинаЧисло для идентификатора |
...
| favorite | Добавил ли пользователь задачу в избранноеtrue — да;false — нет. | Логический |
