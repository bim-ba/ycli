...
- [Формат запроса](https://yandex.ru/support/tracker/ru/api-ref/issues/ru/api-ref/issues/post-attachment#query)
- [Пример запроса](https://yandex.ru/support/tracker/ru/api-ref/issues/ru/api-ref/issues/post-attachment#example)
- [Формат ответа](https://yandex.ru/support/tracker/ru/api-ref/issues/ru/api-ref/issues/post-attachment#answer)

Запрос позволяет прикрепить файл к задаче. Файл появится на странице задачи на вкладке **Вложения**.
...
```
POST /v3/issues/<id_задачи>/attachments/
...
Пример добавления вложенного файла к задаче с помощью curl:

Unix
Windows

```
curl -X POST 'https://api.tracker.yandex.net/v3/issues/<id_задачи>/attachments/' \
...
```
curl.exe -X POST "https://api.tracker.yandex.net/v3/issues/<id_задачи>/attachments/" ^
  -H "Authorization: OAuth y0__xAbc******" ^
  -H "Content-Type: multipart/form-data" ^
  -H "X-Org-ID: 1234******" ^
