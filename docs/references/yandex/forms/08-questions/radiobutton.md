---
source: https://yandex.ru/support/forms/en/blocks-ref/radiobutton
title: "One answer |"
word_count: 513
token_estimate: 865
extracted: "2026-05-22T18:04:59Z"
mode: quality
---

To answer this question, you can select an answer from a list of available options, such as their marital status.

# Question

Enter your question.

-   To add a comment or hint to a question, click **Add explanation**. The text of a comment is displayed using a smaller font size.

    To format the text of a question or comment, use [Markdown markup](https://yandex.ru/support/forms/en/appearance#text-formatting).

-   To add an image to the question, click **Add image**.

-   To make a question required, turn on the ![](https://yandex.ru/support/forms/docs-assets/support-forms/rev/r19697197/en/_assets/enabled-switch-blue.png) **Required** option in the top right corner of the block.

    Required questions are marked with an asterisk (\*) in the form. If a user skips a required question, they won't be able to submit the form.

-   To show or hide a question based on previous answers, click **Display conditions** in the top right corner of the block. For more information, see [Configure display conditions for questions or pages](https://yandex.ru/support/forms/en/add-questions#conditions).

-   To use a question for a test or quiz, click **Score** under **Answers** and enter the number of points that users will get for a correct answer. For more information, see [Conducting a test](https://yandex.ru/support/forms/en/tests).

# Answers

### Appearance

Choose how you want to display the answer options:

Tile

Stars

In one row

Blocks with the answer options are arranged in multiple rows and columns.

Users answer the question by giving a rating represented by the number of stars they choose.

The answer options are arranged in a single row. One row can hold up to 4 options.

### Answer options

Specify options that users can choose from:

-   To add an answer option, click **Answer option** and enter the answer text.

-   You can use [Markdown](https://yandex.ru/support/forms/en/appearance#text-formatting) for formatting.

-   To add a picture to an answer option, click .

-   To relocate an answer option in the list, drag it with .

To reveal additional question settings, click **Show settings** at the top of the question block. To hide the settings, click the icon again.

### Configure answer options

You can set the following for each answer option:

-   **ID**. Used to [prefill the form](https://yandex.ru/support/forms/en/pre-fill). Make sure all answer options in the same form have unique IDs.

-   **Hidden answer**. Enable this option to temporarily hide an answer option without deleting it.

# Settings

To reveal additional question settings, click **Show settings** at the top of the question block. To hide the settings, click the icon again.

### Sort answers

Select a sorting method:

-   **Alphabetical**.
-   **Random for each user**. Select this sorting method if you don't want the order of answers to impact a user's choice.

### Questions IDs

Use the question ID to [prefill the form](https://yandex.ru/support/forms/en/pre-fill).

Question IDs may contain capital and lowercase Latin letters, numbers, `-` and `_` characters. Make sure all questions in the same form have unique IDs.

### Hide question

Enable this option to hide a question in the form. You can use hidden questions to [pass service or auxiliary parameters](https://yandex.ru/support/forms/en/pre-fill#hidden-query).

Don't select both the **Hide question** and **Required** options at the same time. Otherwise, users won't be able to submit the form.