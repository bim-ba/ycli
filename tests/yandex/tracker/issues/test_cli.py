...
def test_list_builds_filter_body():
    ...
    res = runner.invoke(cli.app, ["--format", "json", "tracker", "issues", "list", "--queue", "DE", "--status", "open"])
    assert res.exit_code == 0
...
def test_search_builds_query_body():
    ...
    res = runner.invoke(cli.app, ["--format", "json", "tracker", "issues", "search", "Queue: DE AND Status: open"])
    assert res.exit_code == 0
...
def test_count_filters():
    ...
    res = runner.invoke(cli.app, ["tracker", "issues", "count", "--queue", "DE", "--status", "open"])
    assert res.exit_code == 0
...
def test_create_assembles_body_with_polymorphic_wrap_and_fields():
    ...
    res = runner.invoke(cli.app, [
        "--format", "json", "tracker", "issues", "create",
        "--queue", "DE", "--summary", "New", "--type", "task",
...
def test_update_assembles_partial_body():
    ...
    res = runner.invoke(cli.app, ["--format", "json", "tracker", "issues", "update", "DE-5", "--summary", "U", "--type", "bug"])
    assert res.exit_code == 0
    sent = json.loads(responses.calls[0].request.body)
...
def test_update_assembles_full_body():
    ...
    res = runner.invoke(cli.app, [
        "--format", "json", "tracker", "issues", "update", "DE-5",
        "--summary", "U", "--type", "bug",
        "--priority", "normal", "--parent", "DE-1", "--description", "body",
        "--tag", "a", "--tag", "b", "--field", "sprint=123",
    ])
