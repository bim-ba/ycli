...
def edit(
    ...
    """Edit a sprint SPRINT_ID (PATCH /sprints/{sprint_id}) — only supplied fields are sent."""
    body = SprintUpdate(
        name=name or None,
    ...
    app_ctx = AppContext.from_typer_context(ctx)
    Serializer.serialize(
        app_ctx.tracker.sprints.edit(sprint_id, body), app_ctx.strategy, app_ctx.console
    )
...
@app.command()
def start(ctx: typer.Context, sprint_id: SprintIdArg) -> None:
    """Start a sprint SPRINT_ID (POST /sprints/{sprint_id}/_start; status → in_progress)."""
    app_ctx = AppContext.from_typer_context(ctx)
    Serializer.serialize(
        app_ctx.tracker.sprints.start(sprint_id=sprint_id), app_ctx.strategy, app_ctx.console
    )


@app.command()
def archive(ctx: typer.Context, sprint_id: SprintIdArg) -> None:
    """Archive a sprint SPRINT_ID (POST /sprints/{sprint_id}/_archive; status → archived)."""
    app_ctx = AppContext.from_typer_context(ctx)
    Serializer.serialize(
        app_ctx.tracker.sprints.archive(sprint_id=sprint_id), app_ctx.strategy, app_ctx.console
    )
